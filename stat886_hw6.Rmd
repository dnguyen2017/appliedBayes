...
# theta.init <- rep(100, nschool)
# mu.init <- 100
# tau2.init <- 50

# gibbs_sat <- function(theta.init, mu.init, tau2.init) {
#   
# }
# 
...
for (iter in 2:(n.burn + n.iter)) {
  ...
  theta_var <- 1/(1/sat$se^2 + 1/tau2[iter-1])
  theta[iter,] <- rnorm(nschool, mean = theta_mu_numerator/theta_mu_denominator, sd = sqrt(theta_var))
  
  # sample mu
  theta_bar <- mean(theta[iter - 1,])
  mu[iter] <- rnorm(1, mean = theta_bar, sd = sqrt(tau2[iter-1]/nschool))
  
  # sample tau^2
  # tau2[iter] <- rgamma(1, shape = (nschool -1)/2, scale = 0.5 * sum((theta[iter-1,] - mu[iter-1])^2))
  tau2[iter] <- 1/rgamma(1, shape = (nschool -1)/2, scale = 0.5 * sum((theta[iter-1,] - mu[iter-1])^2))
}
```
...
I do not think these chains have converged. After burn-in, there are cases where the chain jumps to extreme parameter values.
...
I do not think that SAT prep courses are helpful, because the mean treatment effect is centered on zero.
